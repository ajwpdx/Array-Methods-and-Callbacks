...
let final2014 = fifaData.findIndex(
    array => array.Year === 2014 && array.Stage === 'Final'
); //looks up the 2014 final game for reference below.
...
function getWinnersByYear(country, year, data) {
    const allWinners = [];
    getFinals(data).forEach((item, index) =>{
        allWinners.push(`In ${year[index]}, ${country[index]} won the world cup!`)
    })
    return allWinners
};

console.log(getWinnersByYear(getWinners(getFinals,fifaData),getYears(getFinals, fifaData),fifaData));

/* Task 7: Write a function called `getAverageGoals` that accepts a parameter `data` and returns the the average number of home team goals and away team goals scored per match (Hint: use .reduce and do this in 2 steps) */

function getAverageGoals(data){
    const homeGoals = data.reduce(function(acc, cur){
        return acc + cur["Home Team Goals"];
    },0);
    const awayGoals = data.reduce(function(acc, cur){
        return acc + cur["Away Team Goals"];
    },0);
    return `${(homeGoals / data.length).toFixed(2)} Home Goals vs. ${(awayGoals / data.length).toFixed(2)} Away Goals`
};
